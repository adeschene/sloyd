...
├── index.html                    # Task 1
├── package.json / tsconfig / vite.config.ts   # Task 1
...
**Files:**
- Create: `package.json`, `tsconfig.json`, `tsconfig.node.json`, `vite.config.ts`, `index.html`, `src/main.tsx`, `src/App.tsx`, `src/styles.css`, `src/smoke.test.ts`
- Modify: `.gitignore`
...
If it refuses because the directory is non-empty, answer to keep existing files. `CLAUDE.md`, `docs/`, `.git/`, and `.gitignore` must survive.

- [ ] **Step 2: Install runtime and dev dependencies**

```bash
npm install three @react-three/fiber @react-three/drei zustand
npm install -D vitest jsdom @testing-library/react @testing-library/user-event @testing-library/jest-dom @types/three
```
...
`vite.config.ts` now uses the `test` field, which needs Vitest's types. Add to `tsconfig.node.json` `compilerOptions.types`: `["vitest/config"]`.
...
- [ ] **Step 6: Replace App with a placeholder shell**
...
`src/styles.css` — replace contents entirely:
...
Ensure `src/main.tsx` imports `./styles.css` and not the Vite template's `index.css`/`App.css`. Delete `src/index.css`, `src/App.css`, and `src/assets/` if the template created them.
...
export const useStore = create<StoreState>((set, get) => {
  ...
  return {
    ...
    duplicateBoard: (id) => {
      ...
      if (!source) return;
      const copy = createBoard({ ...source, id: undefined, name: `${source.name} copy` });
      edit(
...
Note on `duplicateBoard`: spreading `source` then setting `id: undefined` relies on `createBoard`'s spread overwriting the generated id with `undefined`. That would produce an invalid board — so instead destructure explicitly:

```ts
const { id: _drop, ...rest } = source;
const copy = createBoard({ ...rest, name: `${source.name} copy` });
```

Use this second form. It is the one the tests expect.
...
export function BoardMesh({ board, selected, onSelect }: Props) {
  ...
  return (
    <group position={center}>
      ...
      <lineSegments>
        <edgesGeometry args={[new THREE.BoxGeometry(...extents)]} />
...
Add `import * as THREE from 'three';` at the top. Constructing `BoxGeometry` inline on every render leaks GPU memory; wrap it so it is disposed properly:

```tsx
import { useMemo, useEffect } from 'react';

const edges = useMemo(() => new THREE.EdgesGeometry(new THREE.BoxGeometry(...extents)), [extents[0], extents[1], extents[2]]);
useEffect(() => () => edges.dispose(), [edges]);
```

and render `<lineSegments geometry={edges}><lineBasicMaterial .../></lineSegments>`. Use this memoized form — the inline version above is shown only to make the intent clear.
...
export function Gizmo() {
  ...
  // Keep the invisible proxy object in step with the document. The gizmo
  // drags the proxy; the document is what the drag ultimately writes to.
  useEffect(() => {
    if (board) proxy.current.position.set(...boardCenter(board));
...
6. Dragging, then `Ctrl+Z` later (Task 8) will need to undo cleanly — for now confirm each drag produces store updates by watching React DevTools or a temporary `console.log` in `commit`.

Known wrinkle to watch for: `onObjectChange` fires on every frame of a drag, so each drag pushes many undo entries. Accept that for now — Task 8 Step 7 collapses it.
...
```ts
    beginGesture: () => {
      // Snapshot once, then suppress further pushes until endGesture.
      const { doc, past } = get();
      set({ past: [...past, doc].slice(-HISTORY_LIMIT), future: [] });
      gesturing = true;
    },
    endGesture: () => { gesturing = false; },
```

Wire it in `Gizmo.tsx`: `onMouseDown={() => useStore.getState().beginGesture()}` and change `onMouseUp` to `() => { commit(); useStore.getState().endGesture(); }`.

Wire it in `Toolbar.tsx` for the name field: `onFocus={() => useStore.getState().beginGesture()}` and `onBlur={() => useStore.getState().endGesture()}`. Do the same for the part-name input in `Properties.tsx`.
...
**Known rough edge carried on purpose:** Task 7 leaves per-frame undo entries during a drag; Task 8 Step 7 fixes it, and a test pins the fix. This is called out in Task 7 so a task-scoped reviewer does not flag it as a defect.
