...
**Goal:** Ship a browser-based 3D woodworking modeller where a user adds rectangular boards, sizes and positions them in fractional inches, and saves the result — deployed at `sloyd.example.com`.
...
- Modify: `CLAUDE.md`, `/srv/CLAUDE.md`
...
Expected: one running container. Find its name (likely `sloyd-app-1`) and check it serves:

```bash
docker run --rm --network proxy_network curlimages/curl:latest -sS -o /dev/null -w '%{http_code}\n' http://sloyd-app-1/
docker run --rm --network proxy_network curlimages/curl:latest -sS -o /dev/null -w '%{http_code}\n' http://sloyd-app-1/some/deep/route
```
...
Rewrite `/srv/sloyd/CLAUDE.md`: replace the "Status: pre-v1" section with the shipped state, fill in the empty Architecture section (document-is-truth, the module table, the storage seam), and add a **Manual deployment steps** section stating explicitly that a human must:

1. Create a DNS A record for `sloyd.example.com` pointing at the VPS.
2. In the nginx-proxy-manager admin UI on `:81`, add a proxy host: domain `sloyd.example.com`, forward to hostname `sloyd-app-1`, port `80`, then request a Let's Encrypt certificate.

State that Claude cannot perform either step.

- [ ] **Step 7: Add the hub table row**

In `/srv/CLAUDE.md`, add to the "Projects at a glance" table:

```markdown
| `sloyd/` | **Active — production** | via NPM → `sloyd.example.com` | `sloyd/CLAUDE.md` |
```
