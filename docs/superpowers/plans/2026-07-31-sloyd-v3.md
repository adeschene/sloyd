...
describe('wobbleAt', () => {
  ...
  it('meets at the same slope, so the join is invisible', () => {
    // Central differences, not one-sided. A one-sided estimate carries an O(h)
    // bias of h*f''(z), which here is ~9e-5 — larger than a 4-digit tolerance,
    // so the test would fail against a perfectly correct implementation.
    const h = 1e-5;
    const slopeAt = (z: number) =>
      (wobbleAt(z + h, harmonics) - wobbleAt(z - h, harmonics)) / (2 * h);
    expect(slopeAt(0)).toBeCloseTo(slopeAt(1), 4);
  });
...
In `src/viewport/grainTexture.ts`, delete `woodFace`, `woodEdge` and `woodEnd`. **Keep `streak`** — `plywoodFace` still calls it, and plywood's drawing is explicitly unchanged by this task. Keep `base`, `dash`, `speckle`, `plywoodFace`, `plywoodPlies` and `mdf` exactly as they are. Add:
