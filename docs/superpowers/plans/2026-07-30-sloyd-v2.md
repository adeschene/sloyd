...
describe('updateBoard reorients a board in place', () => {
  ...
  it('turns an already-standing board in place', () => {
    const id = aBoard();
    useStore.getState().updateBoard(id, { standing: true });
    const before = board().position;
    useStore.getState().updateBoard(id, { rotation: 90 });
    // The patch carries no `standing` key. If that absence is passed along as
    // an explicit undefined, the extents are computed for a board that is not
    // standing and the board lands nowhere near its own footprint.
    expect(board().position[1]).toBe(before[1]);
    // Standing at [10, 0, 6.375], extents 24 x 5-1/2 x 3/4 become
    // 3/4 x 5-1/2 x 24: X takes +11-5/8, Z takes -11-5/8.
    expect(board().position).toEqual([21.625, 0, -5.25]);
  });

  it('stands an already-turned board in place', () => {
    const id = aBoard();
    useStore.getState().updateBoard(id, { rotation: 90 });
    const before = board().position;
    useStore.getState().updateBoard(id, { standing: true });
    expect(board().position[1]).toBe(before[1]);
    expect(board().position[0]).toBeCloseTo(before[0] + (5.5 - 0.75) / 2);
    expect(board().position[2]).toBe(before[2]);
  });
...
```ts
    updateBoard: (id, patch) => {
      ...
      // Only the fields the patch actually carries go into `changes`. An
      // explicit `rotation: undefined` would not fall through to the board's
      // own value — the spread inside reorientedPosition would overwrite it —
      // so a patch that toggles standing on an already-turned board would
      // compute its extents as though the board were unturned.
      const changes: { rotation?: Rotation; standing?: boolean } = {};
...
Save this exactly as `/tmp/v1-rotations.sloyd` and open it through the File menu. This
is the migration ordering trap seen from the outside, and it is the most consequential
check in the whole plan — do not paraphrase the file.
...
```json
{
  ...
  "boards": [
    {
      "id": "b_old_180", "name": "Folded 180",
      "length": 24, "width": 5.5, "thickness": 0.75,
      "position": [0, 0, 0], "rotation": 180, "standing": false, "material": "pine"
    },
    {
      "id": "b_old_270", "name": "Folded 270",
      "length": 24, "width": 5.5, "thickness": 0.75,
      "position": [0, 0, 12], "rotation": 270, "standing": false, "material": "walnut"
    }
  ]
}
```

Expected: it opens without error. **Folded 180** lies along X, identical to a `0` board.
**Folded 270** lies along **Z**, identical to a `90` board — if it lies along X, the fold
is running after `validateBoard` and every 270° board in every saved file is being turned
a quarter turn the wrong way. Neither board has moved: their Position fields still read
`0"/0"/0"` and `0"/0"/12"`. Both now show **Along X** and **Along Z** respectively in the
Grain select.
