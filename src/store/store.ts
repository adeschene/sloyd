...
export const useStore = create<StoreState>((set, get) => {
  ...
  return {
    ...
    commitSnapMove: (target) => {
      ...
      // board's points, and every writer of selectedId drops a grab that
      // stops matching (see edit() and selectBoard). This guard is
      // deliberately redundant with both: the filter makes the rule true of
      // the UI, this makes it true of the action, so a future sixth writer of
      // selectedId that misses the rule costs a grab that refuses to commit
      // rather than a board that moves without the user knowing which one it
      // was. `grabbed` is deliberately left in hand — this state should be
      // unreachable once the filter lands, and discarding it quietly would
      // hide that it wasn't.
